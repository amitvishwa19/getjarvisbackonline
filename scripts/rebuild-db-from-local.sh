...
# Extract titles from MEMORY.md Pending Tasks
mapfile -t titles < <(sed -n '/^## Pending Tasks/,/^## /p' "$SCRIPT_DIR/../MEMORY.md" | grep '^- ' | sed 's/^- *//' | sed '/^$/d')

echo "Found ${#titles[@]} titles. Inserting..."

for t in "${titles[@]}"; do
  # Generate description (same as add-todo-auto)
